...
const CoverLetterWriter = () => {
  ...
  const submitFormData = async (e: any) => {
    e.preventDefault();
    setLoading(true);
    const userDetails = e.target[0].value;
    const response = await openai.createCompletion({
      model: "text-davinci-003",
      prompt: `Write a cover letter based on the following information: ${userDetails}`,
      temperature: 0.6,
      max_tokens: 1000,
    });
      ...
      <div className="mt-20">
        <h1 className="text-6xl font-bold">
          <div className="flex bg-white">
            Write your <p className="text-orange-500 pl-5"> Cover letter</p>
          </div>
        </h1>
      </div>
      <main className="relative flex flex-1 flex-col items-center justify-center px-20 text-center bg-slate-800 rounded-md m-20 text-white w-9/12 shadow-md shadow-black">
        {loading ? (
          ...
          <p className="mt-3 text-2xl">
            Write a short message about you, your skills, experience and
            projects.{" "}
          </p>
        )}
